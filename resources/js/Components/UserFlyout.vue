...
<template>
    <div class="h-screen text-white w-full bg-texture-1 overflow-hidden">
        <div class="grid-cols-1 mx-auto w-full h-full py-9 overflow-hidden">
            <div>
            ...
            <div id="userInfoSection" class="grid grid-cols-2 mt-4 mb-5">
                ...
                <form @submit.prevent="form.post('/userUpdate')">
                    ...
                    <div v-if="errors.name" class="text-xs w-40 text-red-300 px-2">{{ errors.name }}</div>
                    <input type="password" name="password" v-model="form.password" placeholder="password" class="bg-transparent rounded-full w-40">
                    <div v-if="errors.password" class="text-xs w-40 text-red-300 px-2">{{ errors.password }}</div>
                    
                    <button type="submit" class="w-40 text-sm rounded-full bg-white/30 ml-4 p-3 border hover:bg-white/40 mt-2 -translate-x-[5rem]"> Update </button>
                </form>
                </div>
            </div>
